...
const regexNomPrenom =
    /^[A-Za-zÀ-ÖØ-öø-ÿ](?:[ -']?(?![ -']{2})[A-Za-zÀ-ÖØ-öø-ÿ])*$/;
const emailRegex = /^[^\s@]+@ccicampus\.fr$/;
const adresseRegex =
    /^[0-9A-Za-zÀ-ÖØ-öø-ÿ][ -']?(?:(?![ -']{2})[0-9A-Za-zÀ-ÖØ-öø-ÿ])*$/;
...
function validateName() {
    ...
    if (firstName === "") {
        ...
        showErrorMessage(
            "Le nom doit comporter entre 2 et 25 caractères !",
            "firstName"
        );
        return false;
    } else if (!regexNomPrenom.test(firstName)) {
        showErrorMessage(
            "Le nom doit commencez par une lettre et ne peut pas avoir 2 caractères spéciaux qui ce suive",
            "firstName"
        );
        return false;
    }
...
function validateAddress() {
    ...
    if (address.length < 2 || address.length > 50) {
        showErrorMessage(
            "L'adresse doit comporter entre 2 et 50 caractères !",
            "address"
        );
        return false;
    }
...
function validateCity() {
    ...
    if (city.length < 2 || city.length > 25) {
        showErrorMessage(
            "La ville doit comporter entre 2 et 25 caractères !",
            "city"
        );
        return false;
    }
    ...
    //     showErrorMessage(
    //         "La ville doit commencez par une lettre et ne peut pas avoir 2 caractères spéciaux qui ce suive",
    //         "city"
    //     );
    //     return false;
    // }
...
function validatePhone() {
    ...
    if (!regexPhone.test(phone)) {
        showErrorMessage(
            "Le numéro de téléphone doit comporter que des chiffres !",
            "phone"
        );
        return false;
    }
...
function validateEmail() {
    ...
    if (!emailRegex.test(email)) {
        showErrorMessage(
            "L'adresse email doit être de type @ccicampus.fr !",
            "email"
        );
        return false;
    }
...
function validatePassword() {
    ...
    if (!passwordRegex.test(password)) {
        showErrorMessage(
            "Le mot de passe doit comporter au moins 12 caractères dont 2 majuscules, 2 minuscules, 2 chiffres et 2 caractères spéciaux",
            "your-password"
        );
        return false;
    }
...
function validateConfirmPassword() {
    ...
    if (password === "" || confirmPassword === "") {
        showErrorMessage(
            "Les deux champs de mot de passe doivent être remplis !",
            "your-confirm"
        );
        return false;
    }
...
function validateTerms() {
    ...
    if (!termsCheckbox.checked) {
        ...
        errorMessageElement.innerHTML =
            "Les termes et conditions sont obligatoires !";
        return false;
    }
...
function validateTime() {
    ...
    if (timeStart === "" || timeEnd === "") {
        ...
        errorMessageElement.innerHTML =
            "Veuillez choisir à la fois l'heure de début et l'heure de fin !";
        return false;
    }
...
document
    .getElementById("your-password")
    .addEventListener("blur", validatePassword);
document
    .getElementById("your-confirm")
    ...
    fetch(
        ...
        .then((data) => {
            if (data && data.length > 0) {
                ...
                document.getElementById("lat").setAttribute("value", latitude.substring(0, 15));
                document.getElementById("lon").setAttribute("value", longitude.substring(0, 15));
            } else {
...
document.querySelector("#address").addEventListener("input", runFunction);
document.querySelector("#zip").addEventListener("input", runFunction);
document.querySelector("#city").addEventListener("input", runFunction);
