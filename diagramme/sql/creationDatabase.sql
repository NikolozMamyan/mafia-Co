...
CREATE DATABASE
    cciCovoiturage DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
...
VALUES (1, 'Admin'), (2, 'Conducteur'), (3, 'Passager'), (4, 'Conducteur / Passager');

INSERT INTO
    `JourSemaine` (idJourSemaine, labelJourSemaine)
VALUES (1, 'Lundi', 'Lun'), (2, 'Mardi', 'Mar'), (3, 'Mercredi', 'Mer'), (4, 'Jeudi', 'Jeu'), (5, 'Vendredi', 'Ven');
...
CREATE OR REPLACE TRIGGER USERMODIFDATEBEFOREUPDATE 
BEFORE UPDATE ON Utilisateurs FOR EACH ROW BEGIN
	SET NEW.derniereModificationutilisateur = NOW();
	END |


CREATE OR REPLACE TRIGGER TRAJETMODIFDATEBEFOREUPDATE 
BEFORE UPDATE ON Itineraire FOR EACH ROW BEGIN
	SET NEW.derniereModificationTrajet = NOW();
	END |

CREATE OR REPLACE TRIGGER NOTIFICATIONREADDATEBEFOREUPDATE
BEFORE UPDATE ON Notifications FOR EACH ROW BEGIN
	IF NEW.isReadNotification <> OLD.isReadNotification
    THEN
